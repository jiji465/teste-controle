...
import { buildSafeDate, isHoliday, isWeekend } from "@/lib/date-utils"
...
export function FiscalCalendar({ obligations, taxes = [], installments = [], clients = [] }: Props) {
  ...
  const allItems = useMemo<CalendarItem[]>(() => {
    ...
    // Impostos com dueDay viram um vencimento mensal NESTE mês
    for (const t of taxes) {
      if (!t.dueDay) continue
      const date = buildSafeDate(year, month, t.dueDay)
      items.push({
        id: `tax-${t.id}-${year}-${month}`,
        kind: "tax",
        name: t.name,
        clientName: "Catálogo de impostos",
        dueDate: date,
        status: t.status,
